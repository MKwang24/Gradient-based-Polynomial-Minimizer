...
public class Minimizer {
	...
	public void minimize(Polynomial p) throws Exception {
		...
		TreeSet <String> a = p.getAllVars();
//		for (String i :a) {
//			gradient.set(i,p.differentiate(i).evaluate(_x0) );
//		}
		for (String i :a) {
				_var2gradp.put(i,p.differentiate(i) );
		...
		while(_nIter<_maxIter &&  _lastGradNorm > _eps) {
			...
			_x0.setAll(_x0.sum(gradient.scalarMult(-1*_stepSize)));
			_lastx.setAll(_x0);
			_lastObjVal =p.evaluate(_x0);
			System.out.format("At iteration %d: %s objective value = %.3f\n", _nIter, _lastx, _lastObjVal);
		}
