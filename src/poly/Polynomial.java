...
public class Polynomial {
	...
	public static Polynomial ReadPolynomial(File file) throws PolyException, IOException {
		if (file.canRead()!=true) {
			throw new PolyException("I cannot read this boi");
		}
		try {
			BufferedReader pen = new BufferedReader(new FileReader(file.getAbsolutePath()));
			String Poly = pen.readLine();
			pen.close();
			return new Polynomial(Poly);
		}
		  catch(IOException e){
			  throw new IOException("IOException: error reading file");
		  }
	...
	public double evaluate(Vector assignments) throws Exception {
		for (Term t : this._terms) {
			for (String key : t.getAllVars()) {
				if (!assignments.contain(key)) {
					throw new PolyException("Variables doesnt match with Vector");
					}
			}	
		}
		//vec vars != p.vars throw Vector Exception 
		//Vector index 'x' not found in { y=2.0000 }
		double result = 0 ;
		for (Term i : _terms) {
			result += i.evaluate(assignments);
		}
		return result;
	}
	...
	public Polynomial differentiate(String var) {
		...
		for (Term i :_terms) {
			Term di = i.differentiate(var);
			if (di.getCoef()!= 0.0d)
				x.addTerm(di);
			//System.out.println(i.differentiate(var));
		}
	...
	public static void main(String[] args) throws Exception {
		...
		//System.out.println(Polynomial.ReadPolynomial(poly1.txt));
	}
}
